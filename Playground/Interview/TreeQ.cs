...
namespace Playground.Interview
{
    public class TreeQ
    {
        ...
        /// implement an efficient function findLargestSmallerKey that finds the largest key in the tree that is smaller than num. If such a number doesn’t exist, return -1. Assume that all keys in the tree are nonnegative.
        /// </summary>
        private int FindLargestSmallerKey(Tree<int>.Node root, int num)
        {
            // Time complexity -> O(h) means that O(n) - worst case
            Tree<int>.Node node = root;
            Tree<int>.Node targetNode = null;
